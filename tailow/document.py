...
class DocumentMeta(type):
    def __new__(mcs, name, bases, attrs):
        ...
        setattr(
            meta_data,
            "_uniques",
            DocumentMeta.get_unique_fields(attrs["_fields"]),
        )
        setattr(ncs, "_ensured_index", False)
        setattr(ncs, "_meta", meta_data)
        setattr(ncs, "_collection", meta_data.name)
        setattr(
            ncs, "objects", classproperty(lambda *arg, **kwargs: QuerySet(ncs))
        )
        return ncs
...
class Document(with_metaclass(DocumentMeta)):
    def __init__(self, **kwargs):
        ...
        for key, value in kwargs.items():
            if key in self._fields:
                if self._fields[key].is_reference and not isinstance(
                    value, Document
                ):
    ...
    def validate(self):
        for field in self._fields:
            if (
                self._fields[field].required
                and self._values.get(field) is not None
            ):
                continue
